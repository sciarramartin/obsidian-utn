  ...
  try {
    ...
    for (const item of req.body.items) {
      if (!mongoose.Types.ObjectId.isValid(item.productoId)) {
        return res.status(400).json({ error: `ID de producto inválido: ${item.productoId}` });
      }
...
router.post('/:id/confirmar', async (req, res) => {
  try {
    ...
    // Regla de Negocio 1: El pedido debe estar pendiente
    if (pedido.estado !== 'pendiente') {
      return res.status(409).json({ error: 'El pedido ya fue confirmado' });
    }

    // Regla de Negocio 2: Verificar stock disponible de todos los ítems
    ...
    // Descontar stock de forma atómica en MongoDB
    for (const item of pedido.items) {
