# 🟢 Hub: Emprendimientos Tecnológicos & FarmaLink

**MOC Principal:** [[00_Indice_Maestro|Mi Segundo Cerebro]]  
**Tags:** #hub #materia/emprendimientos-tecnologicos #utn #sistemas #startup #farmalink  

---

## 🧭 Temas y Metodologías de la Cátedra

* [[2026-08-14_emprendimientos_tecnologicos_utn_frc|Ficha de la Asignatura Emprendimientos]]
* [[2026-08-14_metodologia_organizaciones_exponenciales_exo|Metodología de Organizaciones Exponenciales (ExO) & Launchpad]]
* [[2026-08-28_sprint_1_actividades_metodologia_exo|Sprint 1: Actividades ExO / Lean Startup]]

---

## 💊 Proyecto de Cursada: FarmaLink (Recetas Electrónicas)

* [[2026-08-28_idea_proyecto_plataforma_farmacias_receta_electronica|Idea de Proyecto: FarmaLink]]
* [[2026-08-30_idea_preliminar_plataforma_farmacias_recetas|Alcance, Flujo y Tablero Farmacias / Pacientes]]
* [[2026-08-31_bmc_postits_plataforma_farmacias|Business Model Canvas: Post-its del Tablero FarmaLink]]
* [[2026-09-06_banco_preguntas_encuesta_farmalink|Banco de Preguntas para Encuestas y Validación]]

---

## 📋 Lista de Notas de Emprendimientos (Dataview)

```dataview
TABLE file.mtime as "Modificado"
FROM ""
WHERE contains(file.tags, "emprendimientos-tecnologicos")
SORT file.mtime DESC
```
